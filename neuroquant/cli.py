...
def build_model(config: QuantizationConfig) -> nn.Module:
    ...
    """
    from neuroquant.models.model_loader import ModelLoader
...
def build_data_loaders(config: QuantizationConfig):
    ...
    """
    from neuroquant.data.data_loader import GenericDatasetLoader
...
class NeuroQuantPipeline:
    ...
    def __init__(
        ...
        from neuroquant.utils.common import set_seed
        ...
        # Checkpoint manager
        from neuroquant.utils.checkpointing import CheckpointManager
    ...
    def phase_1c_nsga_search(self) -> None:
        ...
        if getattr(hp, "hardware_aware_search", False):
            try:
                from neuroquant.utils.onnx_export import is_onnx_available
                if is_onnx_available():
                    from neuroquant.quantization.latency_lut import build_latency_lut
    ...
    def phase_1f_gptq_smooth_awq(self) -> None:
        ...
        for i, (key, label, bw, cls) in enumerate(plan, 1):
            ...
            try:
                ...
                if bw == 4 or (bw_assignment and 4 in bw_assignment.values()):
                    try:
                        from neuroquant.utils.onnx_export import estimate_int4_packed_size_mb
        ...
        if "awq" in produced_models:
            from neuroquant.quantization.awq import serialize_awq_metadata
        ...
        if "smoothquant" in produced_models:
            from neuroquant.quantization.smoothquant import (
    ...
    def phase_2_pareto(self) -> None:
        ...
        from neuroquant.visualization.pareto_analysis import ParetoAnalyzer
        from neuroquant.utils.common import compute_quantized_size_mb
    ...
    def _resume_phase_1f_gptq_smooth_awq(self) -> None:
        ...
        if self.ckpt.file_exists("phase_1f_awq_model.pt"):
            from neuroquant.quantization.awq import restore_awq_wrappers
        ...
        if self.ckpt.file_exists("phase_1f_sq_model.pt"):
            from neuroquant.quantization.smoothquant import restore_smoothquant_wrappers
    ...
    def _export_detection_backends(self) -> None:
        ...
        """
        try:
            from neuroquant.utils.deployment_export import (
...
def _run_init_command(force: bool) -> int:
    ...
    ``neuroquant/quantization/_default_config.yaml`` via
    ``importlib.resources`` so it works both from a source checkout and
    from a wheel installed under ``site-packages``.
    """
    ...
    try:
        source_traversable = importlib_resources.files("neuroquant.quantization") / "_default_config.yaml"
