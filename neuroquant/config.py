...
class QuantizationConfig:
    ...
    #   * ``classification`` — runs the standard last-Linear adaptation
    #     + first-Conv stride tweak. Forward outputs ``[B, num_classes]``
    #     logits.
    #   * ``detection`` — loads from ``torchvision.models.detection`` and
    #     skips the classification-specific adapters; head replacement is
    #     handled inline by the loader for the Faster/Mask/Keypoint
    #     R-CNN family. Forward outputs a list of dicts with
    #     ``boxes``/``labels``/``scores``.
    #   * ``segmentation`` — loads from ``torchvision.models.segmentation``
    #     (FCN, DeepLabV3, LRASPP) with ``num_classes`` propagated through
