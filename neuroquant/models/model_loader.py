...
class ModelLoader:
    ...
    def load(self) -> nn.Module:
        ...
        # Step 2: Adapt for target task. Only classification models go
        # through the standard adapters (last-Linear swap, first-Conv
        # stride tweak). Detection models come from
        # ``torchvision.models.detection`` with head replacement
        # handled inline; segmentation models come from
        # ``torchvision.models.segmentation`` already configured for the
        # requested ``num_classes`` via the constructor. Running the
        # classification adapters on those backbones would corrupt FPN
        # heads / segmentation decoders, so we skip them.
    ...
    def _build_base_model(self) -> nn.Module:
        ...
        # Priority 3: Segmentation model (torchvision.models.segmentation)
        if self.config.task == "segmentation":
            return self._load_torchvision_segmentation_model(self.config.model_name)

        # Priority 4: Classification model from torchvision.models
    ...
    def _load_torchvision_detection_model(self, name: str) -> nn.Module:
        ...
        # ── Pretrained path (COCO DEFAULT weights) ──
        # torchvision only ships COCO-pretrained detectors, so a real
        # (non-random) baseline means loading the published 91-class COCO
        # head as-is. We deliberately do NOT pass ``num_classes`` here —
        # it conflicts with the pretrained head — and we skip the
        # box-predictor swap below so the COCO classifier is preserved.
        # This is the only way the FP32 baseline has a meaningful mAP for
        # the quantization study; a randomly-initialised detector scores
        # ~0 and makes the whole comparison vacuous.
        if self.config.model_pretrained:
            logger.info(
                "  Loading PRETRAINED detection weights (COCO DEFAULT) for %s "
                "(config num_classes=%d is informational; the pretrained head "
                "defines the real 91-class COCO space).",
                name_normalised, self.config.num_classes,
            )
            return getattr(tv_detection, name_normalised)(weights="DEFAULT")

        logger.info(
            "  Loading torchvision detection model: %s (num_classes=%d, "
    ...
    def _load_torchvision_segmentation_model(self, name: str) -> nn.Module:
        """
        Load a semantic-segmentation model from
        ``torchvision.models.segmentation``.

        Mirrors ``_load_torchvision_detection_model`` but targets the
        segmentation sub-package (FCN, DeepLabV3, LRASPP, …). The
        constructor receives ``num_classes`` directly so the classifier
        head emits ``[B, num_classes, H, W]`` logits without needing a
        post-hoc head swap. ``aux_loss=False`` keeps the secondary
        classifier off by default — its loss head is only useful during
        training, and including it would force users to provide an extra
        ``"aux"`` ground truth.

        Outputs at inference time are an ``OrderedDict`` whose ``"out"``
        key holds the per-pixel logits — see the XAI module for how the
        Grad-CAM backward pass consumes that.
        """
        if not HAS_TORCHVISION:
            raise ImportError(
                "torchvision is required to load segmentation models by name. "
                "Install it or set config.model_class to a custom model."
            )

        try:
            from torchvision.models import segmentation as tv_segmentation
        except ImportError as exc:  # pragma: no cover — torchvision without segmentation
            raise ImportError(
                "torchvision.models.segmentation unavailable; install a "
                "torchvision build that includes the segmentation module."
            ) from exc

        name_normalised = name.lower().strip()
        if not hasattr(tv_segmentation, name_normalised) or not callable(
            getattr(tv_segmentation, name_normalised)
        ):
            available = [
                n for n in dir(tv_segmentation)
                if not n.startswith("_") and callable(getattr(tv_segmentation, n))
            ]
            raise ValueError(
                f"Unknown segmentation model '{name}'. "
                f"Available torchvision.models.segmentation entries include: "
                f"{', '.join(sorted(available)[:20])}..."
            )
        ...
        # / ``weights_backbone=None`` / ``aux_loss=False`` only when the
        # constructor advertises them. Falls back to the bare
        # ``num_classes`` call if introspection fails.
        import inspect
        try:
            sig = inspect.signature(model_fn)
            kwargs: Dict[str, Any] = {"num_classes": self.config.num_classes}
            for opt in ("weights", "weights_backbone", "aux_loss"):
                if opt in sig.parameters:
                    kwargs[opt] = False if opt == "aux_loss" else None
            return model_fn(**kwargs)
        except Exception as exc:
            logger.warning(
                "  Falling back to bare num_classes kwarg for '%s' (%s).",
                name_normalised, exc,
            )
            return model_fn(num_classes=self.config.num_classes)
